...
class CVDemo():
    ...
    def __init__(self):
        ...
        self.CLASSES = pickle.loads(open("models/coco_classes.pickle", "rb").read())
    ...
    def runInferenceOnFrameUsingSdk(self, frame, config):  
        ...
        self.debug("call convertFrameToTensorDict start ")
        dictTensor = self.convertFrameToTensorDict(frame, config['width'], config['height'])
        # Run inference by calling the wallaroo url rest api endpoint feeding it the jsonInput
        self.debug("call convertFrameToTensorDict end ")
        time.sleep(0.001)     
    ...
    def runInferenceOnFrame(self, frame, config):      
        self.debug("convertFrameToTensorDict start")
        dictTensor = self.convertFrameToTensorDict(frame, config['width'], config['height'])
        # Run inference by calling the wallaroo url rest api endpoint feeding it the jsonInput
        inferResult = None
        json = None
        #try:
        self.debug("convertFrameToTensorDict end")
